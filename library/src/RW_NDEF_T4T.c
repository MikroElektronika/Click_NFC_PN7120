...
*******************************************************************************/
static uint8_t RW_NDEF_T4T_APP_SELECT[] =
{
    0x00, 0xA4, 0x04, 0x00, 0x07, 0xD2, 0x76, 0x00, 0x00, 0x85, 0x01, 0x01
};
static uint8_t RW_NDEF_T4T_CC_SELECT[] =
{
    0x00, 0xA4, 0x00, 0x0C, 0x02, 0xE1, 0x03
};
static uint8_t RW_NDEF_T4T_NDEF_SELECT[] =
{
    0x00, 0xA4, 0x00, 0x0C, 0x02, 0xE1, 0x04
};
static uint8_t RW_NDEF_T4T_READ[] =
{
    0x00, 0xB0, 0x00, 0x00, 0x0F
};

static uint8_t RW_NDEF_T4T_OK[] = {0x90, 0x00};
...
)
{
    ...
    *p_rsp_size = 0;
    ...
    case SELECTING_NDEF_APPLICATION20:
        ...
        if ( !memcmp( &p_cmd[cmd_size - 2], ( uint8_t * )RW_NDEF_T4T_OK, sizeof( RW_NDEF_T4T_OK ) ) )
        {                                                                                                                                                /* Select CC */
    ...
    case READING_CC:
        ...
        {                                                                                                                                                /* Fill CC structure */
            ...
            if( rw_ndef_t4t_ndef.mapping_version == 0x10 )
                    rsp[3] = 0x00;
        ...
        if( !memcmp( &p_cmd[cmd_size - 2], ( uint8_t * )RW_NDEF_T4T_OK, sizeof( RW_NDEF_T4T_OK ) ) )
        {
            ...
            {                                                                                                                                        /* Notify application of the NDEF reception */
                if( p_rw_ndef_pull_cb != NULL )
                        p_rw_ndef_pull_cb( rw_ndef_t4t_ndef.p_record, rw_ndef_t4t_ndef.record_size );
            }
            else
            {                                                                                                                                        /* Read NDEF data */
                memcpy( rsp, ( uint8_t * )RW_NDEF_T4T_READ, sizeof( RW_NDEF_T4T_READ ) );
