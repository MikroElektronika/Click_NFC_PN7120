...
static uint8_t SNEP_PUT_SUCCESS[] = {SNEP_VER10, SNEP_SUCCESS, 0x00, 0x00, 0x00, 0x00};
static uint8_t LLCP_CONNECT_SNEP[] = {0x11, 0x20};
static uint8_t LLCP_I_SNEP_PUT_HEADER[] = {SNEP_VER10, SNEP_PUT, 0x00, 0x00, 0x00, 0x00};
static uint8_t LLCP_SYMM[] = {0x00, 0x00};
...
static void parse_llcp
(
                uint8_t *p_buf,
        p2p_ndef_llcp_header_t *p_llcp_header
);

static void fill_llcp
(
...
void p2p_ndef_register_pull_callback
(
                void *p_cb
)
{
...
{
    p2p_ndef_llcp_header_t llcp_header;
    *p_rsp_size = 0;                                                                                                                        /* Initialize answer */
    parse_llcp( p_cmd, &llcp_header );

    switch ( llcp_header.pdu )
    {
    case CONNECT:                                                                                                                                /* Is connection from SNEP Client ? */

        if ( llcp_header.dsap == SAP_SNEP ){
            if( p_p2p_ndef_pull_cb != NULL ){                                                                        /* Only accept the connection is application is registered for NDEF reception */
                llcp_header.pdu = CC;
            ...
            llcp_header.pdu = DM;                                                                                                /* Refuse any other connection request */
            fill_llcp( llcp_header, p_rsp );
            *p_rsp_size = 2;
        }
        break;
    case I:                                                                                                                                                /* Is SNEP PUT ? */
        ...
        if ( ( p_cmd[3] == SNEP_VER10 ) && ( p_cmd[4] == SNEP_PUT ) ){                /* Notify application of the NDEF reception */
            if( p_p2p_ndef_pull_cb != NULL )
                    p_p2p_ndef_pull_cb( &p_cmd[9], p_cmd[8] );
        ...
        case SNEP_CLIENT_CONNECTED:
            ...
            *p_rsp_size = 9 + ndef_record_size;
            ...
            if( p_p2p_ndef_push_cb != NULL ) 
                p_p2p_ndef_push_cb( p_ndef_record, ndef_record_size );
            break;
        case DELAYING_PUSH:
